...
以上两项均满足，Pilot 才启动 Inspector，并在 gate-c-review.json 中
...
（verdict、rollback_to 等）同步写入 gate-c-review.json 供 Pilot 解析。
```
...
- `.pipeline/artifacts/adr-draft.md`（v4 新增）：Architect 在方案确定后立即产出 ADR 草稿，保留决策理由最鲜活的上下文，Documenter 在 Phase 5 基于此草稿最终化。Pilot 机械验证 adr-draft.md 存在且非空后才放行 Gate A。
...
- 存在 uncovered 假设 → `overall: WARN`，Pilot 将未覆盖假设列表附加到 Gate B 的输入上下文，由 Auditor-Biz 判断是否需要 Planner 补充；不自动阻断（避免过度严苛），但 Auditor-Biz FAIL 时须说明是否因此原因。
...
Pilot 机械生成路径、方法、错误码骨架，Contract Formalizer 只填充 `__FILL__` 字段。Phase 2.6 将机械验证所有 `__FILL__` 均已填充且格式合法。
...
Pilot 在 Phase 3 开始前，根据 tasks.json 计算文件分配矩阵：
...
- `.pipeline/artifacts/gate-c-review.json`（JSON 结论，供 Pilot 流转决策）
...
**服务启动失败处理（v4 修复漏洞 D）：** 启动本地服务前，AutoStep 先执行健康检查（最多等待 30 秒，轮询 `/health` 或配置的 startup_probe）。若服务无法启动，在 `contract-compliance-report.json` 中设置 `startup_error: true`，Pilot 将此情况视为**基础设施故障**（而非契约违规）→ 直接触发 **Escalation**，不执行 Phase 3 回退（服务启动失败通常是环境问题，回退 Builder 无意义）。若服务成功启动后 dredd/schemathesis 测试失败，则按原有规则回退 Phase 3。
...
**ADR 最终化（v4 新增）：** Documenter 读取 Phase 1 Architect 输出的 `adr-draft.md`，补充实现细节、测试结果、实际影响，生成最终 ADR 文件。若 `adr-draft.md` 不存在，Pilot 阻断 Phase 5 并要求 Architect 补充（回退 Phase 1）。
...
| 失败类型 | 判断条件 | 动作 |
|---------|---------|------|
| 部署脚本执行失败（deploy_status: FAIL） | 部署过程报错，未到达 Smoke Test | 若本次变更已部分应用（partial deploy），Pilot 激活 Deployer 执行 `rollback_command` 回滚；重试最多 `max_attempts["phase-6"]` 次；超限 → Escalation |
| Smoke Test 失败（smoke_test: FAIL） | 部署完成但健康检查不通过 | Pilot 激活 Deployer 执行 `rollback_command` 回滚，回滚后进入 Phase 3 排查（此时 state 标记 `mode: post-deploy-smoke-fail`） |
...
| CRITICAL | 任意指标超 critical 阈值，或 service_down | **Pilot 重新激活 Deployer 执行生产环境回滚**（执行 deploy-report.json 中的 rollback_command），回滚成功后进入 Phase 1 重新设计方案 |
...
27. **Contract Formalizer 模板驱动（修复漏洞 I）**：Pilot 从 tasks.json 机械生成 OpenAPI 骨架，LLM 仅填充语义字段，大幅降低格式错误概率，Phase 2.6 复查效果更可预期。
