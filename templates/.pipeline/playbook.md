...
**自治模式**（`autonomous_mode = true`）：spawn 消息首行追加 `[AUTONOMOUS_MODE]`，Clarifier 跳过 Q&A 直接生成 requirement.md（所有不确定项用 `[ASSUMED]` 标注）。
...
写日志：调用 `write_step_log`，step=`"gate-a"`，step_type=`"gate"`，agent=`"auditor"`，从 `gate-a-review.json` 提取 `overall`、`rollback_to` 及所有 `severity=CRITICAL` 的 `issues[].description` 作为 `key_decisions`。
...
写日志：调用 `write_step_log`，step=`"gate-b"`，step_type=`"gate"`，agent=`"auditor"`，从 `gate-b-review.json` 提取 `overall`、`rollback_to` 及所有 `severity=CRITICAL` 的 `issues[].description` 作为 `key_decisions`。
...
写日志：调用 `write_step_log`，step=`"gate-c"`，step_type=`"gate"`，agent=`"inspector"`，从 `gate-c-review.json` 提取 `overall`、`rollback_to` 及所有 `severity=CRITICAL` 的 `issues[].description` 作为 `key_decisions`。
...
写日志：调用 `write_step_log`，step=`"gate-d"`，step_type=`"gate"`，agent=`"auditor-qa"`，从 `gate-d-review.json` 提取 `overall`、`rollback_to` 及所有 `severity=CRITICAL` 的 `issues[].description` 作为 `key_decisions`。
...
写日志：调用 `write_step_log`，step=`"gate-e"`，step_type=`"gate"`，agent=`"auditor-qa+tech"`，从 `gate-e-review.json` 提取 `overall`、`rollback_to` 及所有 `severity=CRITICAL` 的 `issues[].description` 作为 `key_decisions`。
...
> **注意**：此表为通用参考。各阶段的**具体提取字段名**以上方对应章节的"写日志"指令为准。
