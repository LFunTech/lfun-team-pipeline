...
**依赖:** 无，本文件是第一步。
**后续:** Part 2（Phase 0-2 agents）→ Part 3（Phase 3-7 agents）→ Part 4（AutoStep scripts）→ Part 5（Templates + install.sh）
...
**Files:**
- Create: `agents/` 目录
- Create: `templates/.pipeline/autosteps/` 目录
- Create: `templates/.pipeline/artifacts/.gitkeep`

**Step 1: 创建所需目录**

```bash
mkdir -p agents
mkdir -p templates/.pipeline/autosteps
mkdir -p templates/.pipeline/artifacts
touch templates/.pipeline/artifacts/.gitkeep
```

**Step 2: 验证目录存在**

```bash
ls agents/ && ls templates/.pipeline/
```
Expected: `autosteps/  artifacts/`

**Step 3: Commit**

```bash
git add templates/
git commit -m "feat: initialize agents/ and templates/.pipeline/ directory structure"
```
...
> 注意：Gate A 在 Phase 1 之后（Phase 0 → Phase 0.5 → Phase 1 → Gate A）。
...
```
FAIL → rollback_to: phase-3（指定越权 builder）
...
- `deployment_failed` → rollback_to: phase-3（重做实现）
