...
找到现有说明：
```
**关于新增测试文件的生命周期：** Tester 新增的测试文件在 `impl-manifest.json` 中标记为 `"new_test_files": [...]`。Phase 3.3 / Phase 3.6 的 Regression Guard 排除这些文件。若 Phase 4a FAIL 回退到 Phase 3，Orchestrator 将 `new_test_files` 列表保留在 state.json 中，Builder 修复后 Phase 3.3 / 3.6 仍排除它们，避免未修复的新测试被纳入回归守卫形成死锁。
```
...
Read Gate D 章节全文，确认：①两个 JSON 场景均有 rollback_to 字段；②Orchestrator 验证说明存在；③与 rollback_to 范围限制说明（"不得指定 phase-0 或 phase-1"）一致。
...
**Orchestrator 写入 state.json（v6 明确，修复漏洞 W）：** AUTOSTEP_API_CHANGE_DETECTOR 完成后，Orchestrator 读取 `api-change-report.json.phase_5_mode`，同步写入 `state.json.phase_5_mode`。两个文件均保留此字段：`api-change-report.json` 作为产物归档，`state.json` 作为运行时状态（Documenter 运行时的读取来源）。
```
